...
-(void)drawTopBarWords{
	...
	NSString *title_Month   = [[NSString alloc] initWithFormat:@"%@ %d",[self getMonth:currentMonthDate.month ],(int) currentMonthDate.year];
	...
	[@"Wed" drawAtPoint:CGPointMake(s_width*3+9,fontsize) withFont:weekfont];
	[@"Thurs" drawAtPoint:CGPointMake(s_width*4+9,fontsize) withFont:weekfont];
...
-(void)drawGirdLines{
	...
	CGContextRef ctx=UIGraphicsGetCurrentContext();
	int width=self.frame.size.width;
	int row_Count=([self getDayCountOfaMonth:currentMonthDate]+[self getMonthWeekday:currentMonthDate]-1)/7+1;
...
-(void)drawDateWords{
	...
	for(int i=1;i<dayCount+1;i++)
	{
		...
		if([self getDayFlag:i]==1)
		{
			...
			[@"." drawAtPoint:CGPointMake(x*s_width+19,y*itemHeight+headHeight+40) withFont:[UIFont boldSystemFontOfSize:25]];
		}
...
- (void) touchAtDate:(CGPoint) touchPoint{
    ...
    //return in NSDate format....
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    [formatter setDateFormat:@"yyyy-MM-dd"];
    NSString* day = [NSString stringWithFormat:@"%ld-%d-%d", currentSelectDate.year, currentSelectDate.month, currentSelectDate.day];
    NSDate* d = [formatter dateFromString:day];
    ...
    NSString *weekDay =  [theDateFormatter stringFromDate:d];
    
    //the day is for deterimning if this route runs today
    int dayInt = [self getIntFromStringDay:weekDay];

    
   // NSLog(@"%d, %@", dayInt, weekDay);
